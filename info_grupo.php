<?php
	...
	if(isset($_SESSION['nombre_usuario']) && ($_SESSION['tipo']==1 || $_SESSION['tipo']==2 || $_SESSION['tipo']==3)){/*SI EL QUE INGRESO A NUESTRA PAGINA ES CONSULTOR DE CUALQUIER TIPO*/
		$home="";
		...
		?>
				...
				<div class="box-content padding-in" style="text-align:left;" >
					<?php
						...
						$consulta_int = mysql_query($consulta_1,$conn) or 
												die("Could not execute the select query.");
						...
						$consulta_grupo = mysql_query($consulta_2,$conn) or 
												  die("Could not execute the select query.");
				...
				<div class="box-content padding-in"  >
					<?php
						$consulta_representante = mysql_query("SELECT id_usuario,nombre, apellido, telefono, codigo_sis, nombre_carrera, usuario.nombre_usuario, email,habilitado, grupo_empresa 
															   FROM integrante, usuario, carrera 
															   WHERE integrante.usuario=$id_usuario AND  usuario.id_usuario=integrante.usuario AND carrera=carrera.id_carrera",$conn) or 
														  die("Could not execute the select query.");
						...
						$consulta_grupo = mysql_query("SELECT nombre_largo, nombre_corto, abreviatura, nombre,apellido, u.habilitado
													   FROM grupo_empresa g,sociedad s, usuario u
													   WHERE g.id_grupo_empresa=$id_grupo_empresa AND g.sociedad=s.id_sociedad AND g.consultor_tis=u.id_usuario",$conn) or 
												  die("Could not execute the select query.");
						$resultado = mysql_fetch_assoc($consulta_grupo);
